...
def run_multihead_self_attention(
    ...
    """
    rope = None
    multi_self_attention = MultiHeadSelfAttention(d_input=d_model,
                                                  num_heads=num_heads,
                                                  d_qk=d_model,
                                                  d_v=d_model,
                                                  d_output=d_model,
                                                  rope=rope)
    multi_self_attention.load_state_dict(
        {
    ...
    return multi_self_attention(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    return multi_self_attention(in_features,token_positions)
...
def run_rmsnorm(
    ...
    rmsnorm.load_state_dict({"gates":weights})
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    in_features.transpose_(dim,-1)
    safe_softmax = SafeSoftmax()
    in_features = safe_softmax(in_features)
    in_features.transpose_(dim,-1)
    return in_features
...
def get_tokenizer(
    ...
    """
    tokenizer = Tokenizer()
    tokenizer.init_from_given_member(vocab, merges, special_tokens)
    return tokenizer
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    """
    tokenizer = Tokenizer()
    tokenizer.train(datafile=input_path,
                    special_tokens=special_tokens,
                    vocab_end_size=vocab_size
                    )
    vocab = {}
    for i,b in enumerate(tokenizer.token_id_to_bytes):
        vocab[i] = b
